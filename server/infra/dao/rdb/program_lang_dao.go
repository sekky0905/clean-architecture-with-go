...
func NewProgrammingLangDAO(ctx context.Context, manager SQLManagerInterface) repository.ProgrammingLangRepository {
...
func (dao *ProgrammingLangDAO) Create(lang *model.ProgrammingLang) (*model.ProgrammingLang, error) {
	query := "INSERT INTO programming_languages (name, feature, created_at, updated_at) VALUES (?, ?, ?, ?)"

	stmt, err := dao.SQLManager.PrepareContext(dao.Ctx, query)
	if err != nil {
		return nil, dao.ErrorMsg(model.DBMethodCreate, err)
	}
	defer stmt.Close()

	result, err := stmt.ExecContext(dao.Ctx, lang.Name, lang.Feature, lang.CreatedAt, lang.UpdatedAt)
	if err != nil {
		return nil, dao.ErrorMsg(model.DBMethodCreate, err)
	}
...
func (dao *ProgrammingLangDAO) List(limit int) ([]*model.ProgrammingLang, error) {
	query := "SELECT id, name, feature, created_at, updated_at FROM programming_languages ORDER BY name LIMIT=?"
	return dao.list(query, limit)
}

// list は、レコードの一覧を取得して返す。
func (dao *ProgrammingLangDAO) list(query string, limit int) ([]*model.ProgrammingLang, error) {
	...
	rows, err := stmt.QueryContext(dao.Ctx, limit)
	if err != nil {
		return nil, dao.ErrorMsg(model.DBMethodList, err)
	}
	defer rows.Close()
...
func (dao *ProgrammingLangDAO) Read(id int) (*model.ProgrammingLang, error) {
	...
	row := stmt.QueryRowContext(dao.Ctx, id)
	lang := &model.ProgrammingLang{}
...
func (dao *ProgrammingLangDAO) ReadByName(name string) (*model.ProgrammingLang, error) {
	query := "SELECT id, name, feature, created_at, updated_at FROM programming_languages WHERE name=? ORDER BY name LIMIT=?"
	langSlice, err := dao.list(query, 1)
	if err != nil {
...
func (dao *ProgrammingLangDAO) Update(lang *model.ProgrammingLang) (*model.ProgrammingLang, error) {
	...
	result, err := stmt.ExecContext(dao.Ctx, lang.Name, lang.Feature, lang.CreatedAt, lang.UpdatedAt, lang.ID)
	if err != nil {
		return nil, dao.ErrorMsg(model.DBMethodUpdate, err)
	}
...
func (dao *ProgrammingLangDAO) Delete(id int) error {
	...
	result, err := stmt.ExecContext(dao.Ctx, id)
	if err != nil {
		return dao.ErrorMsg(model.DBMethodDelete, err)
	}
